...
// ================= Global Sync Config (GitHub as database) =================
// الزوار يقرون من site-data.json (عالمي)، والادمن ينشر عليه بزر واحد.
// Token ينحفظ بس بمتصفح الادمن، ما يترفع ابد.
const GLOBAL_SYNC = {
    owner: "U9955",
    repo: "tiger-team-official",
    branch: "main",
    file: "site-data.json",
    tokenKey: "TIGER_GITHUB_TOKEN"
};
...
class CosmicApp {
    constructor() {
        this.STORAGE_KEY = 'COSMIC_PORTAL_EN_V13';
        ...
        this.bindEvents();
        this.renderAll();
        // حمّل النسخة العالمية بعد العرض الفوري (حتى الزوار يشوفون تعديلات الادمن)
        this.loadGlobalData();
    }
    ...
    // ================= Global Sync (site-data.json) =================
    // الزوار: يقرون العالمي تلقائيا. الادمن: يعدل محلي وبعدين ينشر للكل.
    async loadGlobalData() {
        try {
            const res = await fetch(GLOBAL_SYNC.file + '?t=' + Date.now(), { cache: 'no-store' });
            if (!res.ok) return;
            const global = await res.json();
            if (!global || (!global.programs && !global.links && !global.profile)) return;
            const normalized = {
                profile: { ...DEFAULT_DATA.profile, ...(global.profile || {}) },
                programs: global.programs || DEFAULT_DATA.programs,
                links: global.links || []
            };
            // اذا العالمي يختلف عن المحلي، اعتمده وحدّث العرض
            if (JSON.stringify(normalized) !== JSON.stringify(this.data)) {
                this.data = normalized;
                try { localStorage.setItem(this.STORAGE_KEY, JSON.stringify(this.data)); } catch (e) {}
                this.renderAll();
                if (this.adminOverlay && this.adminOverlay.classList.contains('active')) {
                    this.renderAdminProgramsList();
                    this.renderAdminLinksList();
                    this.populateProfileForm();
                }
            }
            this.updateSyncStatus('global');
        } catch (e) {
            console.warn('Global load skipped:', e);
        }
    }

    getGitHubToken() {
        return (localStorage.getItem(GLOBAL_SYNC.tokenKey) || '').trim();
    }
    ...
    updateSyncStatus(mode) {
        ...
        if (!token) {
            el.innerHTML = '⚠️ ضيف GitHub Token حتى تكدر تنشر للجميع. بدون توكن الحفظ يبقى بس بجهازك.';
            el.style.color = '#f59e0b';
            return;
        }
        if (mode === 'published') {
            el.innerHTML = '✅ تم النشر! الزوار راح يشوفون التعديل بعد 1-2 دقيقة (وقت تحديث Vercel).';
            el.style.color = '#10b981';
        } else if (mode === 'global') {
            el.innerHTML = '🌍 متصل بالنسخة العالمية. أي نشر جديد راح يشوفوه كل الزوار.';
            el.style.color = '#38bdf8';
        } else {
            el.innerHTML = '✏️ عندك تعديل محلي. اضغط «نشر للجميع» حتى يشوفوه الزوار.';
    ...
    async publishGlobal() {
        ...
        try {
            ...
            console.error(e);
            this.showToast(e.message || 'فشل النشر', 'error');
        } finally {
            if (btn) { btn.disabled = false; btn.innerHTML = oldText || '<i class="fa-solid fa-earth-americas"></i> نشر للجميع'; }
        }
    }
    ...
    initElements() {
        ...
        this.publishGlobalBtn = document.getElementById('publishGlobalBtn');
        this.reloadGlobalBtn = document.getElementById('reloadGlobalBtn');
    ...
    bindEvents() {
        ...
        // Global Sync: save token / publish / reload
        if (this.saveTokenBtn) {
            this.saveTokenBtn.addEventListener('click', () => {
                const v = (this.githubTokenInput.value || '').trim();
                localStorage.setItem(GLOBAL_SYNC.tokenKey, v);
                this.updateSyncStatus();
                this.showToast(v ? 'تم حفظ التوكن بنجاح ✦' : 'تم مسح التوكن');
            });
        }
        if (this.publishGlobalBtn) {
            this.publishGlobalBtn.addEventListener('click', () => this.publishGlobal());
        }
        if (this.reloadGlobalBtn) {
            this.reloadGlobalBtn.addEventListener('click', () => {
                this.loadGlobalData();
                this.showToast('تم تحديث النسخة العالمية 🌍');
            });
        }
    ...
    openAdminPanel() {
        ...
        if (this.githubTokenInput) this.githubTokenInput.value = this.getGitHubToken();
        this.updateSyncStatus();
    ...
    renderProfile() {
        ...
        document.title = `${cleanName} | T1GER`;
    }
